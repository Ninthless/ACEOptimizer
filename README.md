...
Releases require Azure Artifact Signing and an Ed25519 update-signing key. Configure the existing Azure signing secrets and add `NETSPARKLE_PRIVATE_KEY` as a GitHub Actions secret containing the Base64 private key generated by NetSparkle.
...
Each release includes a signed `appcast.xml`, its detached `appcast.xml.signature`, the signed installer, and the portable archive. The application verifies the appcast and installer with Ed25519, then verifies the installer's Windows Authenticode trust before installation.

To create a new release:

1. Update the version in `ACEOptimizer.csproj`.
2. Confirm the Azure signing secrets and `NETSPARKLE_PRIVATE_KEY` are configured.
3. Commit and push to `main`.
4. Create and push a tag matching the project version.
5. The workflow validates the tag, runs tests, signs the application and installer, generates the signed update feed, and publishes a GitHub Release.
